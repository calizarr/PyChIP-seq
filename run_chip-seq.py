...
def main():
    ...
    if args.bed:
        ...
        cmd = "for x in {bam_dir}/*.Aligned.out.bam;do y=$(basename ${{x%.*}});bam2bed < $x > {outdir}/$y.bed & done;wait;echo Converting to Bed done".format(**bam2bed)
        cmdlist.append(cmd)
        output_bed = os.path.join(output_base, "Bed_chr")
        ...
        try:
            ...
            if not os.path.isdir(output_base):
                raise
        bed2bed = {'bed_dir': output_base, 'original': "Bd"}
        cmd = "for x in {bed_dir}/*.bed;do y=$(basename ${{x%.*}});sed -- 's/{original}/chr/g' $x > $y.sicer.bed & done;wait;echo Converting to Bed chromosome done".format(**bed2bed)
        cmdlist.append(cmd)
    if args.sicer_rb:
        ...
        bed_dir = Config.get("ALL", "bed_dir")
        files = (glob.glob(os.path.join(bed_dir,"*.sicer.bed")))
        for f in files:
            dic = {"input":f, "conf":args.config}
            cmd = "python run_sicer-rb.py -input_file {input} -config {conf}".format(**dic)
            # cmd = logcommand.format(insert=cmd)
            cmdlist.append(cmd)
    if args.sicer:
